/**
 * Spec §16.9 acceptance: sprint intake from release notes and CHG, impact
...
import { runSprintIntake, runScheduledIntake } from "@/modules/platform-sprints/intake";
import { evaluateReleaseNotesMissing, evaluateUatBeforeProd } from "@/modules/alerting/evaluators/platform";
import { syncRuleCatalogue } from "@/modules/alerting/engine";

const p = () => db.client;
const add = (m: string, data: Record<string, unknown>) => p()[m].create({ data });
const FIXTURE = readFileSync("src/__tests__/fixtures/synthetic/CONFIRM-PLATFORM-RELEASE-SAMPLE.md", "utf8");

type Call = { method: string; path: string; body: Record<string, unknown> | null; query: Record<string, string> };
const state = { calls: [] as Call[], page: { version: 1, body: FIXTURE }, n: 0, changeUpdated: "2026-10-01T09:00:00.000+0000" };
...
function stub() {
  vi.stubGlobal("fetch", vi.fn(async (input: URL | string, init?: RequestInit) => {
    ...
    if (c.path === "/wiki/rest/api/content/search") return json({ results: [
      { id: "900", title: "[Platform] Sprint 9.99 Release Notes", version: { number: state.page.version }, _links: { webui: "/spaces/PREL/pages/900" } },
      { id: "901", title: "[Platform] Sprint X.XX Release Notes – Template", version: { number: 4 } },
    ] });
    if (c.path === "/wiki/rest/api/content/900") return json({ id: "900", title: "[Platform] Sprint 9.99 Release Notes", version: { number: state.page.version }, body: { storage: { value: state.page.body } } });
    if (c.path === "/rest/api/3/search/jql") {
      const jql = String(c.body?.jql);
      if (jql.includes("CHG")) return json({ issues: [
        { key: "CHG-10", fields: { summary: "Platform Sprint 9.99 Upgrade in UAT", updated: state.changeUpdated, created: "2026-09-30T09:00:00.000+0000", resolutiondate: "2026-10-01T09:00:00.000+0000" } },
        { key: "CHG-11", fields: { summary: "Platform Sprint 9.99 Release in PROD (2026-10-20)", updated: state.changeUpdated } },
        { key: "CHG-12", fields: { summary: "Platform Sprint 9.98 Upgrade in UAT", updated: state.changeUpdated } },
      ], isLast: true });
      if (jql.includes("PDEF")) return json({ issues: [
        { key: "PDEF-500", fields: { summary: "Withdrawal screen rounding", reporter: { accountId: "a1", emailAddress: "ann@k.com" } } },
        { key: "PDEF-501", fields: { summary: "Raised by someone else", reporter: { accountId: "a2", emailAddress: "dev@platform.example" } } },
      ], isLast: true });
      return json({ issues: [], isLast: true });
    }
    if (c.method === "POST" && c.path === "/rest/api/3/issue") return json({ id: String(++state.n), key: `OPS-${state.n}` }, 201);
...
const change = async (summaryPart: string) => (await p().platformChange.findMany({})).find((c) => String(c.summary).includes(summaryPart))!;
...
beforeEach(async () => {
  ...
  state.n = 0;
  stub();
  await add("jiraProjectConfig", { key: "OPS", name: "Transaction Operations", enabled: true, issueTypeIds: { _default: "10001", Task: "10001" } });
  ...
  ]) await add("platformImpactRule", r);
  ...
  for (const code of ["ALR-UAT-01", "ALR-UAT-02", "ALR-UAT-04", "ALR-UAT-05", "ALR-HB-RELNOTES"]) await p().alertRule.update({ where: { code }, data: { enabled: true } });
});
...
describe("sprint intake", () => {
  it("reads CHG dates and the release notes, skipping the template page", async () => {
    const res = await runSprintIntake();
    expect(res.sprints).toEqual(["9.99"]);
    const sprint = await p().platformSprint.findUnique({ where: { sprint: "9.99" } });
    expect(sprint).toMatchObject({ pageVersion: 1, releaseNotesPageId: "900", changeKeys: ["CHG-10", "CHG-11"], fixVersions: ["9.99.0-alpha.1", "9.99.0-rc.1"] });
  ...
  it("maps and qualifies items per the section rules and impact mapping", async () => {
    ...
    expect((await change("Verify fix PDEF-500")).qualifies).toBe(true);
    expect((await p().platformChange.findMany({})).some((c) => String(c.summary).includes("PDEF-501"))).toBe(false);
  ...
  it("creates the parent, one child per qualifying item, and follow-ups; content, labels and due date as specified", async () => {
    ...
    expect(summaries[0]).toBe("Platform Sprint 9.99 — Transaction Operations UAT");
    const qualifying = (await p().platformChange.findMany({ where: { qualifies: true } }));
    ...
    expect(stakingTicket.labels).toEqual(expect.arrayContaining(["uat", "platform-sprint-9-99", "staking_change"]));
    ...
    expect(text).toContain("Test in Platform UAT only");
    ...
    expect(summaries).toEqual(expect.arrayContaining([
      "[UAT 9.99] Check custody API spec version and re-run connector contract tests",
    ...
    // Ops PIC "Ann Operator" is assigned in Jira; Platform keys are linked.
    expect(state.calls.some((c) => c.method === "PUT" && c.path.endsWith("/assignee"))).toBe(true);
    expect(state.calls.filter((c) => c.path === "/rest/api/3/issueLink").map((c) => (c.body!.outwardIssue as { key: string }).key)).toEqual(expect.arrayContaining(["PDEV-1201", "PREL-88"]));
  ...
  it("re-reading an unchanged page creates nothing", async () => {
    await runSprintIntake();
    const before = { items: (await p().platformChange.findMany({})).length, tickets: created().length };
    await runSprintIntake();
    await runSprintIntake({ force: true });
    expect((await p().platformChange.findMany({})).length).toBe(before.items);
  ...
  it("an edited row updates its ticket with a comment; a removed row is marked removed and never closed", async () => {
    ...
    state.page = {
      version: 2,
      body: FIXTURE.replace("Unstake queue now batched hourly", "Unstake queue now batched every 30 minutes").replace("| Auto-approval threshold | Threshold now per asset | PREL-88 |\n", ""),
    };
    const res = await runSprintIntake();
    expect(res).toMatchObject({ updatedItems: 1, removedItems: 1 });
    expect(created().length).toBe(tickets);
    const edited = (await p().platformChange.findMany({ where: { predecessorId: staking.id } }))[0];
    ...
    expect((await p().platformChange.findUnique({ where: { id: risk.id } }))!.removedAt).toBeInstanceOf(Date);
...
describe("alerts", () => {
  it("ALR-UAT-02 fires at PROD minus two business days, not before", async () => {
    ...
    for (const c of await p().platformChange.findMany({ where: { qualifies: true } })) await p().platformChange.update({ where: { id: c.id }, data: { uatOutcome: "pass" } });
    expect(await evaluateUatBeforeProd(ctx("2026-10-16T08:00:00Z"))).toEqual([]);
  });

  it("ALR-HB-RELNOTES: a CHG sprint with no release notes after a day", async () => {
    await runSprintIntake();
    const ctx = (iso: string) => ({ code: "ALR-HB-RELNOTES", now: new Date(iso), params: { graceHours: 24 } }) as never;
...
describe("schedule", () => {
  it("the hourly job runs the full intake on the configured schedule or when CHG changes", async () => {
    expect((await runScheduledIntake(new Date("2026-10-02T08:00:00Z"))).trigger).toBe("schedule");
    expect((await runScheduledIntake(new Date("2026-10-02T09:00:00Z"))).ran).toBe(true); // first CHG check records the cursor
    expect((await runScheduledIntake(new Date("2026-10-02T10:00:00Z"))).ran).toBe(false);
    state.changeUpdated = "2026-10-02T10:30:00.000+0000";
    expect((await runScheduledIntake(new Date("2026-10-02T11:00:00Z"))).trigger).toBe("chg");
...
describe("no false green in the intake summary", () => {
  it("counts only tickets Jira accepted; failures are reported separately", async () => {
    await p().jiraProjectConfig.update({ where: { key: "OPS" }, data: { enabled: false } });
    const res = await runSprintIntake();
