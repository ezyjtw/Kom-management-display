/**
 * Spec §16.5, 16.7, 16.8, 16.9 (and 16.4): UAT outcome capture, PDEF defects
 * confirmed by the tester, the /platform-sprints view and run permissions, sprint
 * metrics, the static "no Platform execution" check, and the custody spec checks.
 */
...
import { GET as defectGet, POST as defectPost } from "@/app/api/platform-sprints/defect/[workItemId]/route";
import { GET as sprintsGet } from "@/app/api/platform-sprints/route";
import { POST as runPost } from "@/app/api/platform-sprints/run/route";
import { syncRuleCatalogue } from "@/modules/alerting/engine";
import { signOffAt, sprintUat } from "@/modules/metrics/definitions";
import { checkContract, diffSpecs, type OpenApiDoc } from "@/lib/integrations/custody-api/contract";
...
beforeEach(async () => {
  ...
  vi.stubGlobal("fetch", vi.fn(async (input: URL | string, init?: RequestInit) => {
    ...
    if (init?.method === "POST" && url.pathname === "/rest/api/3/issue") return json({ id: "1", key: "PDEF-77" }, 201);
  ...
  await add("platformSprint", { id: "sp", sprint: "9.99", prodPlannedAt: new Date("2026-10-20T00:00:00Z") });
  await add("workItem", { id: "uat-1", kind: "uat_task", title: "[UAT 9.99] staking_change: ETH", taskCode: "UAT", sourceSystem: "platform_sprint", sourceId: "9.99:c1", clockStartedAt: new Date(), ticketKey: "OPS-10", ticketSystem: "jira", state: "owned", metadata: { platformChangeId: "c1", platformSprint: "9.99", affectedTasks: ["CHK-16"] } });
  await add("platformChange", { id: "c1", sprintId: "sp", section: "Stake / Unstake", itemType: "staking_change", summary: "ETH: Unstake queue", detail: {}, rowHash: "h", qualifies: true, workItemId: "uat-1", uatTicketKey: "OPS-10", platformJiraKeys: ["PDEV-1"] });
  await syncRuleCatalogue();
  await p().alertRule.update({ where: { code: "ALR-UAT-03" }, data: { enabled: true } });
});
afterEach(() => vi.unstubAllGlobals());

const writeUp = (uat?: Record<string, unknown>) => ({ resolutionNote: "Tested in Platform UAT against the batched unstake queue.", rootCause: "product_behaviour", riskScore: "low", uat });
...
describe("UAT outcome capture (spec §16.5)", () => {
  ...
  it("a fail without a linked PDEF ticket is rejected with 422 (acceptance test)", async () => {
    const res = await closeRoute(req("POST", writeUp({ outcome: "fail", evidence: "screenshot-1" })), ctx({ id: "uat-1" }));
    expect(res.status).toBe(422);
    expect(JSON.stringify(await res.json())).toMatch(/PDEF defect/);
    expect(calls.some((c) => c.path.endsWith("/transitions") && c.method === "POST")).toBe(false);
  });

  it("a fail with its PDEF defect closes, records the outcome, links the defect and raises ALR-UAT-03", async () => {
    const res = await closeRoute(req("POST", writeUp({ outcome: "fail", evidence: "screenshot-1", defectKey: "pdef-77" })), ctx({ id: "uat-1" }));
    expect(res.status).toBe(200);
    expect((await p().platformChange.findUnique({ where: { id: "c1" } }))!.uatOutcome).toBe("fail");
    expect((await p().workItem.findUnique({ where: { id: "uat-1" } }))!.metadata).toMatchObject({ uatOutcome: "fail", uatEvidence: "screenshot-1", platformDefectDefectKey: "PDEF-77" });
    expect(calls.some((c) => c.path === "/rest/api/3/issueLink" && JSON.stringify(c.body).includes("PDEF-77"))).toBe(true);
    expect((await p().alert.findMany({ where: { ruleCode: "ALR-UAT-03" } }))).toHaveLength(1);
  });

  it("the PDEF defect is drafted, and only created when the tester confirms", async () => {
    ...
    expect(draft.description).toContain("PDEV-1");
    ...
    expect((await defectPost(req("POST", { ...draft, confirm: true }), ctx({ workItemId: "uat-1" }))).status).toBe(409); // PDEF not configured
    await add("jiraProjectConfig", { key: "PDEF", name: "Platform Service Management", enabled: false, issueTypeIds: { Bug: "10004" } });
    const res = await defectPost(req("POST", { ...draft, confirm: true }), ctx({ workItemId: "uat-1" }));
    expect(res.status).toBe(201);
    expect((await res.json()).data.key).toBe("PDEF-77");
    const create = calls.find((c) => c.method === "POST" && c.path === "/rest/api/3/issue")!;
    expect((create.body!.fields as { project: { key: string } }).project.key).toBe("PDEF");
    expect((await p().workItem.findUnique({ where: { id: "uat-1" } }))!.metadata).toMatchObject({ platformDefectDefectKey: "PDEF-77" });
  });
});

describe("/platform-sprints (spec §16.7)", () => {
  it("shows the gate and lets only leads and admins run the intake", async () => {
    ...
    await p().platformChange.update({ where: { id: "c1" }, data: { uatOutcome: "pass" } });
    ...
    expect((await run.json()).data.skipped.join(" ")).toMatch(/Confluence|CHG/);
  });
});

describe("Platform sprint metrics (spec §16.8)", () => {
...
describe("no Platform execution (spec §16.9 static check)", () => {
  const dir = "src/modules/platform-sprints";
  ...
  it("only Confluence GET, Jira search/GET and Jira issue/comment/link writes are used", () => {
    for (const { f, text } of sources) {
      ...
      expect(text, `${f} imports the custody client`).not.toMatch(/from\s+"[^"]*(custody-api|integrations\/custody)/);
...
describe("custody API spec checks (spec §16.4)", () => {
