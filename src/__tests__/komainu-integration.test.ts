/**
 * Komainu API connector (spec §8.1) against a stubbed fetch.
 */
...
const BASE = "https://api-demo.komainu.io";
type Call = { url: URL; method: string; auth: string | null; body: string | null };
let calls: Call[];

function stubKomainu(routes: (url: URL) => unknown) {
  calls = [];
...
async function freshClient() {
  ...
  return import("@/lib/integrations/komainu-api/client");
}

beforeEach(() => {
  for (const k of Object.keys(envVars)) delete envVars[k];
  envVars.KOMAINU_API_BASE_URL = BASE;
  vi.clearAllMocks();
  prismaMock.sourceHeartbeat.findUnique.mockResolvedValue(null);
});
afterEach(() => {
  vi.unstubAllGlobals();
  delete process.env.KOMAINU_API_SECRET_UK;
  delete process.env.KOMAINU_API_SECRET_JE;
});
...
describe("credentials (CONFIRM-API-SCOPE)", () => {
  it("parses several users and looks secrets up by reference only", async () => {
    ...
    const secrets: Record<string, string> = { KOMAINU_API_SECRET_UK: "s-uk", KOMAINU_API_SECRET_JE: "s-je" };
    const creds = parseCredentials({
      KOMAINU_API_CREDENTIALS: JSON.stringify([
        { label: "uk", user: "u-uk", secretRef: "KOMAINU_API_SECRET_UK" },
        { label: "je", user: "u-je", secretRef: "KOMAINU_API_SECRET_JE" },
      ]),
  ...
  it("refuses secretRefs outside KOMAINU_API_SECRET_* and skips missing secrets", async () => {
    const { parseCredentials } = await freshClient();
    expect(parseCredentials({ KOMAINU_API_CREDENTIALS: JSON.stringify([{ label: "x", user: "u", secretRef: "DATABASE_URL" }]), lookupSecret: () => "leak" })).toEqual([]);
    expect(parseCredentials({ KOMAINU_API_CREDENTIALS: JSON.stringify([{ label: "x", user: "u", secretRef: "KOMAINU_API_SECRET_X" }]), lookupSecret: () => undefined })).toEqual([]);
  });

  it("falls back to the single user/secret pair", async () => {
    const { parseCredentials } = await freshClient();
    expect(parseCredentials({ KOMAINU_API_USER: "u", KOMAINU_API_SECRET: "s", lookupSecret: () => undefined })).toEqual([{ label: "default", user: "u", secret: "s" }]);
  });
});

describe("client", () => {
  it("uses a separate token per API user and follows paging", async () => {
    envVars.KOMAINU_API_CREDENTIALS = JSON.stringify([
      { label: "uk", user: "u-uk", secretRef: "KOMAINU_API_SECRET_UK" },
      { label: "je", user: "u-je", secretRef: "KOMAINU_API_SECRET_JE" },
    ]);
    process.env.KOMAINU_API_SECRET_UK = "s-uk";
    process.env.KOMAINU_API_SECRET_JE = "s-je";
    stubKomainu((url) => {
...
describe("pollers", () => {
  beforeEach(() => {
    envVars.KOMAINU_API_USER = "u";
    envVars.KOMAINU_API_SECRET = "s";
  });
  ...
  it("stores pending requests with only the needed fields and marks vanished ones", async () => {
    ...
    await freshClient();
    const { pollRequests } = await import("@/modules/integrations/komainu/pollers");

    const result = await pollRequests();
    expect(result.count).toBe(1);

    const upsert = prismaMock.sourceRecord.upsert.mock.calls[0][0];
    expect(upsert.where.source_kind_externalId).toEqual({ source: "komainu_api", kind: "request", externalId: "req-1" });
  ...
  it("does not keep addresses or hashes from transactions", async () => {
    ...
    await freshClient();
    const { pollTransactions } = await import("@/modules/integrations/komainu/pollers");
  ...
  it("maps settlement statuses; unmapped ones become unknown and raise ALR-CFG-02 only when enabled", async () => {
    ...
    await freshClient();
    const { pollCollateral } = await import("@/modules/integrations/komainu/pollers");
