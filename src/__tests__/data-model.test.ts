...
const PHASE2_MODELS = [
  ...
  "SourceHeartbeat", "DailyCheckDefinition", "TimeLog", "TicketLink", "IaiDraft",
];
...
describe("migrations", () => {
  ...
  it("are numbered consecutively and Phase 2 starts after Phase 0's 0019", () => {
    ...
    expect(names.slice(19)).toEqual([
      ...
      "0023_source_heartbeats",
      "0024_daily_check_definitions",
      "0025_seed_sla_policies",
    ]);
  });

  it("0020 creates an inactive system actor for 'system' audit writes", () => {
    const sql = readMig("0020_system_audit_actor");
    expect(sql).toMatch(/INSERT INTO "Employee"[\s\S]*'system'[\s\S]*false/);
    expect(sql).toContain("ON CONFLICT");
  });

  it("0022 backfills existing alerts before making ruleCode/dedupeKey required", () => {
    const sql = readMig("0022_alert_engine_fields");
    const backfill = sql.indexOf('SET "ruleCode" = "type", "dedupeKey" = "id"');
    const notNull = sql.indexOf('ALTER COLUMN "ruleCode" SET NOT NULL');
    expect(backfill).toBeGreaterThan(-1);
    expect(notNull).toBeGreaterThan(backfill);
  });

  it("0022 allows only one open alert per (ruleCode, dedupeKey)", () => {
    expect(readMig("0022_alert_engine_fields")).toMatch(
      /CREATE UNIQUE INDEX[^;]*"Alert"\("ruleCode", "dedupeKey"\) WHERE "status" <> 'resolved'/,
    );
  });

  it("0024 drops the one-run-per-day rule and makes items unique per (definition, period)", () => {
    const sql = readMig("0024_daily_check_definitions");
    expect(sql).toContain('DROP INDEX IF EXISTS "DailyCheckRun_date_key"');
    expect(sql).toMatch(/UNIQUE INDEX[^;]*"DailyCheckItem"\("definitionCode", "periodKey"\)/);
  });

  it("0025 seeds SLA policies with only the two known targets set (CONFIRM-SLA-TARGETS)", () => {
    const sql = readMig("0025_seed_sla_policies");
    const rows = [...sql.matchAll(/\('slapol_\w+', '([A-Z0-9-]+)', '[^']*', (NULL|\d+), (NULL|\d+), (NULL|\d+), (NULL|'[^']*'), '([^']+)'/g)]
      .map(([, code, own, first, resolve, rule, cal]) => ({ code, own, first, resolve, rule, cal }));
    expect(rows.map((r) => r.code)).toEqual([
      "CLIENT-Q-P0", "CLIENT-Q-P1", "CLIENT-Q-P2", "CLIENT-Q-P3", "OES-FAIL", "FAB-ACK", "MTD-BREAK",
    ]);
  ...
  it.each(["0021_clients_and_work_items", "0022_alert_engine_fields", "0023_source_heartbeats", "0024_daily_check_definitions"])(
    "%s is re-runnable (start.sh retries failed migrations)",
    (name) => {
      ...
      expect(sql).not.toMatch(/^ALTER TABLE "\w+" ADD CONSTRAINT/m);
    },
  );
});
