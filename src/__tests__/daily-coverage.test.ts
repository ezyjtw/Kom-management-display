 ...
 * team boards, the approved-validator banner and restricted RLS checks.
 */
...
describe("item generation", () => {
  ...
  it("creates one CHK-10 item per settlement window that has started today", async () => {
    ...
    await add("oesWindow", { exchange: "exch-b", cron: "0 */6 * * *", durationMins: 30, referenceTz: "UTC", isActive: true });
    await generateDailyItems(new Date("2026-09-23T09:10:00Z"));
    expect((await items("CHK-10")).map((i) => i.periodKey).sort()).toEqual([
      "2026-09-23:exch-a:09:00Z", "2026-09-23:exch-b:00:00Z", "2026-09-23:exch-b:06:00Z",
    ]);
  });

  it("creates no BANK items while module.bank is off", async () => {
    await generateDailyItems(WED);
    expect(await items("TASK-BANK-REPORT")).toHaveLength(0);
    flags.on.add("module.bank");
    await generateDailyItems(WED);
    expect(await items("TASK-BANK-REPORT")).toHaveLength(1);
  });
...
describe("automated data pulls", () => {
  ...
  it("CHK-01 proposes one exception per stuck transaction and suppresses known-degraded assets with the reason", async () => {
    await add("sourceHeartbeat", { source: "custody_api.transactions", expectedEveryMins: 2, lastSuccessAt: mins(WED, -1), lastCount: 3 });
    ...
    const tx = (id: string, asset: string, age: number) => add("sourceRecord", { source: "custody_api", kind: "transaction", externalId: id, status: "PENDING", occurredAt: mins(WED, -age), fields: { asset } });
  ...
  it("the operator can pass using the collected evidence (required fields checked)", async () => {
    await add("sourceHeartbeat", { source: "custody_api.transactions", expectedEveryMins: 2, lastSuccessAt: mins(WED, -1), lastCount: 0 });
    const it = await item("CHK-01");
  ...
  it("CHK-03 counts requests by type and age band and proposes the old ones", async () => {
    await add("sourceHeartbeat", { source: "custody_api.requests", expectedEveryMins: 1, lastSuccessAt: WED, lastCount: 3 });
    const req = (id: string, status: string, age: number) => add("sourceRecord", { source: "custody_api", kind: "request", externalId: id, status, occurredAt: mins(WED, -age), fields: { type: "CREATE_TRANSACTION" } });
  ...
  it("CHK-10 summarises one window against the expected portfolios", async () => {
    await add("sourceHeartbeat", { source: "custody_api.collateral", expectedEveryMins: 10, lastSuccessAt: WED, lastCount: 3 });
    ...
    await add("sourceRecord", { source: "custody_api", kind: "settlement", externalId: "s1", status: "DONE", mappedStatus: "completed", occurredAt: new Date("2026-09-23T09:02:00Z"), fields: { exchange: "exch-a", portfolio_id: "pf-1" } });
    const it = await item("CHK-10", "2026-09-23:exch-a:09:00Z");
    ...
    expect(r!.exceptions).toEqual([{ summary: "No settlement for portfolio in the exch-a 09:00Z window", reference: "pf-2" }]);
  });

  it("CHK-16 flags staked > total (position check)", async () => {
    await add("sourceHeartbeat", { source: "custody_api.eod_balances", expectedEveryMins: 1440, lastSuccessAt: WED, lastCount: 2 });
    await add("sourceRecord", { source: "custody_api", kind: "eod_balance", externalId: "w1:2026-09-22", lastSeenAt: WED, fields: { asset: "ADA", staked_balance: 120, total_balance: 100 } });
    await add("sourceRecord", { source: "custody_api", kind: "eod_balance", externalId: "w2:2026-09-22", lastSeenAt: WED, fields: { asset: "ADA", staked_balance: 50, total_balance: 100 } });
  ...
  it("CHK-04 counts unscreenable and staking-excluded transactions separately", async () => {
  ...
  it("reports the pull as unavailable (not a pass) when the source has never been polled", async () => {
    const it = await item("CHK-01");
    const r = await collectForItem(it.id as string, WED);
    expect(r).toMatchObject({ available: false, reason: "No successful the custody provider transactions poll yet" });
  });
...
describe("team boards", () => {
  it("shows the owning team's cards plus all-team tasks, with known issues and the Confluence placeholder", async () => {
    await generateDailyItems(WED);
    const board = await buildBoard("Team 1", { now: WED, canViewRealisations: false });
    ...
    expect(chk01.knownIssues).toEqual([]); // deployment data, none shipped
  ...
  it("shows the approved-validator banner on CHK-08 until the set is populated", async () => {
    await syncDailyCheckDefinitions();
    let card = (await buildBoard("Team 1", { now: MON, canViewRealisations: false })).find((c) => c.code === "CHK-08")!;
    expect(card.banners).toContain(APPROVED_VALIDATORS_MISSING);
    await add("approvedValidator", { chain: "ethereum", validator: "v-1" });
    card = (await buildBoard("Team 1", { now: MON, canViewRealisations: false })).find((c) => c.code === "CHK-08")!;
    expect(card.banners).not.toContain(APPROVED_VALIDATORS_MISSING);
  });

  it("hides restricted RLS items without realisation:view", async () => {
    await generateDailyItems(WED);
    const hidden = (await buildBoard("Team 1", { now: WED, canViewRealisations: false })).find((c) => c.code === "CHK-09K")!;
    expect(hidden.items).toHaveLength(0);
    expect(hidden.banners).toContain("Restricted: requires realisation:view.");
    const shown = (await buildBoard("Team 1", { now: WED, canViewRealisations: true })).find((c) => c.code === "CHK-09K")!;
    expect(shown.items).toHaveLength(1);
  });

  it("realisation:view is admins plus named users", async () => {
    expect(await canViewRealisations({ id: "u1", role: "admin" })).toBe(true);
    expect(await canViewRealisations({ id: "u2", role: "lead" })).toBe(false);
    await add("appSetting", { key: "realisation.viewerUserIds", value: ["u2"] });
    expect(await canViewRealisations({ id: "u2", role: "lead" })).toBe(true);
  });
});
