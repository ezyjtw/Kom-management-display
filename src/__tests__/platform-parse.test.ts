...
import { jiraKeysIn, markdownToBlocks, parseReleaseNotes, rowHashOf, sprintFromTitle, storageToBlocks } from "@/modules/platform-sprints/parse";

const md = readFileSync("src/__tests__/fixtures/synthetic/CONFIRM-PLATFORM-RELEASE-SAMPLE.md", "utf8");
...
describe("release-notes parsing", () => {
  ...
  it("keeps the row cell by cell, with summary, environment and Jira keys", () => {
    ...
    expect(toggle.platformJiraKeys).toEqual(["PDEV-1201"]);
    const [ops, client] = byType("ui_change");
    expect(ops.env).toBe("UAT");
    expect(client.env).toBe("both");
    expect(byType("risk_engine_change")[0].platformJiraKeys).toEqual(["PREL-88"]);
  });
  ...
  it("reads the same structure from Confluence storage format", () => {
    const html = `<h2>1.1 Function Released (but disabled or recently enabled in PROD)</h2>
      ...
      <tr><td><p>Bulk withdrawal limits</p></td><td><p>Not started</p></td><td><p>Ann Operator</p></td><td><p>Disabled &amp; pending <ac:structured-macro ac:name="jira"><ac:parameter ac:name="key">PDEV-1201</ac:parameter></ac:structured-macro></p></td></tr>
    ...
    expect(parsed.rows[0].cells.Comments).toBe("Disabled & pending PDEV-1201");
    expect(parsed.rows[0].platformJiraKeys).toEqual(["PDEV-1201"]);
  ...
  it("sprint number from the page title; Jira keys exclude non-Jira tokens", () => {
    expect(sprintFromTitle("[Platform] Sprint 6.19 Release Notes")).toBe("6.19");
    expect(sprintFromTitle("[Platform] Sprint X.XX Release Notes – Template")).toBeNull();
    expect(jiraKeysIn("see PDEF-12, SHA-256 and UTF-8, PREL-7")).toEqual(["PDEF-12", "PREL-7"]);
  });
});
