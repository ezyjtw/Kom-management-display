...
/**
 * Platform sprints (spec §16.7): UAT landed and PROD planned dates, change items by type,
 * mapping to tasks, UAT ticket status and outcomes, and the gate. Testing happens
 * in Platform UAT by people; this page only tracks tickets.
 */

import { useCallback, useEffect, useState } from "react";
import Link from "next/link";
import { GitBranch, RefreshCw, ExternalLink } from "lucide-react";
import type { sprintsView } from "@/modules/platform-sprints/view";
...
export default function PlatformSprintsPage() {
  ...
  const load = useCallback(async () => {
    const json = await fetch("/api/platform-sprints").then((r) => r.json()).catch(() => null);
    if (json?.success) {
  ...
  async function run(force: boolean) {
    setBusy(true);
    const res = await fetch("/api/platform-sprints/run", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ force }) });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl md:text-2xl font-bold text-foreground flex items-center gap-2"><GitBranch size={24} className="text-primary" /> Platform sprints</h1>
          <p className="text-xs text-muted-foreground mt-1">UAT tickets from the Platform release notes. Tests run in Platform UAT, by people.</p>
        </div>
        {canRun && (
