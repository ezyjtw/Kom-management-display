...
/**
 * Bank repo, MVP0 (spec §12 TASK-BANK). Draft process, not operational.
 * The register records what was received and sent; it sends nothing to BANK.
 */
...
export default function BankPage() {
  ...
  const load = useCallback(async () => {
    const json = await fetch("/api/bank").then((r) => r.json()).catch(() => null);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl md:text-2xl font-bold text-foreground flex items-center gap-2"><Landmark size={22} className="text-primary" /> Bank repo (MVP0)</h1>
      <p role="note" className="p-3 rounded-lg text-sm bg-amber-500/10 text-amber-400 border border-amber-500/20 flex gap-2"><AlertTriangle size={16} className="shrink-0" /> The MVP0 settlement process is draft, not operational, and the outbound maker role is unresolved. This register records messages and settlements; it sends nothing to BANK.</p>
      ...
      <section className="bg-card border border-border rounded-xl p-4 space-y-3">
        ...
        <form className="flex gap-2 flex-wrap" onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); void send("/api/bank/instructions", "POST", {
          ...
          <tbody>{(data?.instructions ?? []).map((i) => (
            <tr key={i.id} className="border-t border-border/50">
              ...
              <td className="space-x-1">
                {i.ackStatus === "none" && <>
                  <button onClick={() => void send(`/api/bank/instructions/${i.id}`, "PATCH", { ackStatus: "ACK" }, "ACK recorded.")} className="text-emerald-400">ACK sent</button>
                  <button onClick={() => void send(`/api/bank/instructions/${i.id}`, "PATCH", { ackStatus: "NACK" }, "NACK recorded.")} className="text-red-400">NACK sent</button>
                </>}
                {i.ackStatus === "NACK" && !i.correctedByRef && <button onClick={() => { const ref = prompt("Reference of the corrected instruction:"); if (ref) void send(`/api/bank/instructions/${i.id}`, "PATCH", { correctedByRef: ref }, "Correction recorded."); }} className="text-primary">Corrected by…</button>}
              </td>
            </tr>
          ))}</tbody>
        </table>
      ...
      <section className="bg-card border border-border rounded-xl p-4 space-y-3">
        ...
        <form className="flex gap-2 flex-wrap" onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); void send("/api/bank/settlements", "POST", {
        ...
        <table className="w-full text-xs">
          ...
          <tbody>{(data?.settlements ?? []).map((l) => (
            <tr key={l.id} className="border-t border-border/50">
              ...
              <td>{(l.kytStatus === "fail" || l.kytStatus === "escalated") && <button onClick={() => void send(`/api/bank/settlements/${l.id}`, "PATCH", { kytStatus: "cleared" }, "Compliance outcome recorded.")} className="text-emerald-400">Compliance cleared</button>}</td>
            </tr>
          ))}</tbody>
        </table>
      </section>
      ...
      <section className="bg-card border border-border rounded-xl p-4 space-y-3">
        ...
        <form className="flex gap-2 flex-wrap" onSubmit={(e) => { e.preventDefault(); const f = new FormData(e.currentTarget); void send("/api/bank/fee-balances", "POST", { walletRef: f.get("walletRef"), asset: f.get("asset"), balance: String(f.get("balance") ?? "").trim() }, "Balance recorded."); }}>
