...
/** KPS, K4 realisation and K3 return of assets (spec §12 CHK-09K). RESTRICTED: kps:view only. */
...
export default function KpsPage() {
  const [data, setData] = useState<{ cases: KpsCase[]; notice: string } | null>(null);
  const [denied, setDenied] = useState(false);

  useEffect(() => {
    void fetch("/api/kps").then(async (r) => {
  ...
  if (denied) return <p className="text-sm text-muted-foreground flex items-center gap-2"><Lock size={16} /> Restricted: this view requires kps:view.</p>;

  return (
    <div className="space-y-4">
      <h1 className="text-xl md:text-2xl font-bold text-foreground flex items-center gap-2"><Lock size={22} className="text-primary" /> KPS realisations and returns (restricted)</h1>
