/** POST /api/gx-sprints/run { force? } — "Run sprint intake" (spec §16.1), leads and admins; audited fail-closed. */
...
import { runSprintIntake } from "@/modules/gx-sprints/intake";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const result = await auditedAction(
      { action: "gx_sprint_intake_run", entityType: "gx_sprint", entityId: "all", userId: actor.userId, summary: `Run GX sprint intake${parsed.data.force ? " (re-read all pages)" : ""}`, metadata: actor.metadata },
    ...
    return handleApiError(error, "gx sprint intake run");
  }
}
