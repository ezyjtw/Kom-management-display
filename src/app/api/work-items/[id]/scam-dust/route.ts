...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await prisma.workItem.update({ where: { id }, data: { metadata: next as Prisma.InputJsonValue } });
    const actor = auditActor(auth);
    await createAuditEntry({ action: "scam_dust_case_updated", entityType: "work_item", entityId: id, userId: actor.userId, summary: `Scam/dust case ${item.ticketKey ?? id} updated`, after: parsed.data, metadata: actor.metadata });
