/**
 * GET  /api/platform-sprints/defect/:workItemId — pre-filled PDEF defect for a failed UAT item.
 * POST /api/platform-sprints/defect/:workItemId { summary, description, confirm: true } — create it after
...
import { createDefect, defectDraft, DefectError, defectSchema } from "@/modules/platform-sprints/outcome";
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ workItemId: string }> }) {
  ...
  try {
    ...
    return mapError(error, "platform defect draft");
  }
}
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ workItemId: string }> }) {
  ...
  try {
    ...
    const created = await auditedAction(
      { action: "platform_defect_created", entityType: "work_item", entityId: workItemId, userId: actor.userId, summary: `Create PDEF defect: ${parsed.data.summary}`, metadata: actor.metadata },
      () => createDefect(workItemId, parsed.data),
      (r) => ({ key: r.key }),
    );
    return apiSuccess(created, undefined, 201);
  } catch (error) {
    return mapError(error, "platform defect create");
  }
}
