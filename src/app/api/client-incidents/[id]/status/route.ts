...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Client-visible status change on the JSM request: fail-closed audit.
    const item = await auditedAction(
      { action: "client_status_changed", entityType: "work_item", entityId: id, userId: actor.userId, summary: `Client-visible status -> ${parsed.data.status}`, after: { status: parsed.data.status }, metadata: actor.metadata },
      () => setClientStatus(id, parsed.data.status),
    );
