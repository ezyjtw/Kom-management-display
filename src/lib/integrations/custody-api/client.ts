/**
 * Read-only custody API v1.6.0 client (H2, spec §8.1).
 *
 ...
 *
 * Multiple API users are supported (CONFIRM-API-SCOPE): CUSTODY_API_CREDENTIALS
 * is a JSON array of {label, user, secretRef}, where secretRef names the env
 * var holding that user's secret (CUSTODY_API_SECRET_<SUFFIX>). Without it the
 * single CUSTODY_API_USER / CUSTODY_API_SECRET pair is used.
 */
...
import type { CustodyPagedResponse, CustodyRequest, CustodyTransaction } from "./types";

export type { CustodyPagedResponse, CustodyRequest, CustodyTransaction } from "./types";

export class ForbiddenMethodError extends Error {
  constructor(method: string, path: string) {
    super(`custody API client is read-only: ${method.toUpperCase()} ${path} is not permitted`);
...
export class ForbiddenPathError extends Error {
  constructor(path: string) {
    super(`custody API path not in allowlist: ${path}`);
...
const credentialsSchema = z.array(
  z.object({
    ...
    user: z.string().min(1),
    secretRef: z.string().regex(/^CUSTODY_API_SECRET_[A-Z0-9_]+$/, "secretRef must name a CUSTODY_API_SECRET_* env var"),
  }),
).min(1);

/** Parse credentials from config. Secrets are looked up by reference and never logged. */
export function parseCredentials(cfg: {
  CUSTODY_API_CREDENTIALS?: string;
  CUSTODY_API_USER?: string;
  CUSTODY_API_SECRET?: string;
  lookupSecret: (name: string) => string | undefined;
}): CustodyCredential[] {
  if (cfg.CUSTODY_API_CREDENTIALS?.trim()) {
    try {
      const parsed = credentialsSchema.safeParse(JSON.parse(cfg.CUSTODY_API_CREDENTIALS));
      if (!parsed.success) {
        logger.error("CUSTODY_API_CREDENTIALS is invalid", { issues: parsed.error.issues.map((i) => i.message) });
        return [];
      }
      return parsed.data.flatMap((c) => {
        const secret = cfg.lookupSecret(c.secretRef);
        if (!secret) {
          logger.error("custody API credential secret not set", { label: c.label, secretRef: c.secretRef });
          return [];
        }
        return [{ label: c.label, user: c.user, secret }];
      });
    } catch {
      logger.error("CUSTODY_API_CREDENTIALS is not valid JSON");
      return [];
    }
  }
  if (cfg.CUSTODY_API_USER && cfg.CUSTODY_API_SECRET) {
    return [{ label: "default", user: cfg.CUSTODY_API_USER, secret: cfg.CUSTODY_API_SECRET }];
  }
  return [];
}
...
export function getCredentials(): CustodyCredential[] {
  return parseCredentials({
    CUSTODY_API_CREDENTIALS: env("CUSTODY_API_CREDENTIALS"),
    CUSTODY_API_USER: env("CUSTODY_API_USER"),
    CUSTODY_API_SECRET: env("CUSTODY_API_SECRET"),
...
async function getAccessToken(baseUrl: string, cred: CustodyCredential): Promise<string> {
  ...
  // Spec §17.7: every use of a custody API credential is audit-logged (label only, never the user or secret).
  void recordCredentialUse({ connector: "custody_api", credentialLabel: cred.label, workload: env("KOM_WORKLOAD") });
  if (!res.ok) throw new Error(`the custody provider auth failed for ${cred.label}: ${res.status}`);
...
const breaker = () => CircuitBreaker.for("custody_api", { callTimeoutMs: 90_000 });

export async function custodyRequest<T>(
  method: string,
  path: string,
  ...
  if (!baseUrl || !cred) {
    throw new Error(
      "custody API not configured: CUSTODY_API_BASE_URL and CUSTODY_API_USER/CUSTODY_API_SECRET (or CUSTODY_API_CREDENTIALS) are required",
    );
  }
  ...
  return breaker().execute(async () => {
    ...
    if (!res.ok) throw new Error(`custody API error: ${res.status} ${res.statusText}`);
    return res.json() as Promise<T>;
  });
}

export function custodyGet<T>(path: string, params?: Record<string, string>, credential?: CustodyCredential): Promise<T> {
  return custodyRequest<T>("GET", path, params, credential);
}
...
export async function fetchAllPages<T>(
  ...
  for (let page = 1; page <= MAX_PAGES; page++) {
    const res = await custodyGet<CustodyPagedResponse<T>>(path, { ...params, page: String(page), page_size: "100" }, credential);
    out.push(...(res.data ?? []));
    if (!res.has_next) return out;
  }
  logger.warn("the custody provider paging stopped at MAX_PAGES", { path, pages: MAX_PAGES });
  return out;
}
...
export async function forEachCredential<T>(
  fn: (cred: CustodyCredential) => Promise<T[]>,
...
export async function fetchPendingTransactions(
  ...
  return custodyGet("/v1/custody/transactions", params);
}
...
export async function fetchPendingRequests(
  ...
  return custodyGet("/v1/requests", params);
}
...
export async function fetchTransaction(transactionId: string): Promise<CustodyTransaction> {
  return custodyGet(`/v1/custody/transactions/${encodeURIComponent(transactionId)}`);
}

export async function fetchRequest(requestId: string): Promise<CustodyRequest> {
  return custodyGet(`/v1/requests/${encodeURIComponent(requestId)}`);
}
