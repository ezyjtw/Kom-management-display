/**
 * Komainu API contract (spec §8.1, §16.4): the endpoints and response fields
...
import { AUTH_TOKEN_PATH, KOMAINU_ENDPOINTS } from "@/lib/integrations/komainu-api/endpoints";

/** Response fields the app reads, by list endpoint (items of `data`). TODO(CONFIRM-KOMAINU-OPENAPI): confirm against the spec. */
...
export function checkContract(doc: OpenApiDoc): ContractIssue[] {
  ...
  for (const ep of KOMAINU_ENDPOINTS) {
