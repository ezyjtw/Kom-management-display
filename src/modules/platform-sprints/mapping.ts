 ...
 * PlatformImpactRule rows (config, not code).
 */

import type { PlatformImpactRule } from "@prisma/client";
import type { ItemType, ParsedRow } from "@/modules/platform-sprints/parse";
...
export interface MappingConfig {
  rules: Array<Pick<PlatformImpactRule, "id" | "name" | "matchOn" | "pattern" | "taskCodes" | "alertCodes" | "controls" | "team" | "uatTemplate" | "priority">>;
...
function ruleMatches(rule: MappingConfig["rules"][number], row: ParsedRow): boolean {
  ...
  switch (rule.matchOn) {
    ...
    case "jira_project": return row.platformJiraKeys.some((k) => re.test(k.split("-")[0]));
...
function qualifiesBySection(type: ItemType, row: ParsedRow, cfg: MappingConfig, mappedByWorkstream: boolean): { yes: boolean; tags: string[]; reason: string } {
  switch (type) {
    ...
    case "verify_fix":
      return { yes: true, tags: [], reason: "PDEF ticket raised by Transaction Operations in the sprint label: verify the fix" };
...
export function classify(row: ParsedRow, cfg: MappingConfig): Classification {
  ...
  if (WALLET_TECH.test(row.text)) {
    ...
    if (!cfg.includeWalletTech) {
      qualifies = false;
      reason = "New wallet technology: scoped, not operational (enable platform.uat.include_wallet_tech to test)";
    }
  }
