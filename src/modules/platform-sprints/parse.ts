 ...
 * "[Platform] Sprint X.XX Release Notes – Template"; they are parsed
...
function toRow(section: string, itemType: ItemType, cells: Record<string, string>): ParsedRow {
  ...
  return { section, itemType, cells, text, summary, firstCell, env: envOf(cells), platformJiraKeys: jiraKeysIn(text), rowHash: rowHashOf(section, cells) };
}
  ...
  for (const b of blocks) {
    ...
    for (const r of body) {
      ...
      if (type === "metadata") {
        ...
        // TODO(CONFIRM-PLATFORM-RELEASE-SAMPLE): where the planned PROD date sits in the Versions & Artifacts table.
...
/** Sprint number from a page title like "[Platform] Sprint 6.19 Release Notes". */
