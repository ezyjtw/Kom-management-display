/**
 * Komainu API pollers (spec §8.1). Each poll aggregates across every
...
import { mapStatuses, type StatusEntity } from "@/modules/integrations/komainu/status-map";

export const SOURCE = "komainu_api";

/** Heartbeat sources and their expected cadence (minutes). */
export const KOMAINU_HEARTBEATS = {
  requests: { source: "komainu_api.requests", expectedEveryMins: 1 },
  transactions: { source: "komainu_api.transactions", expectedEveryMins: 2 },
  collateral: { source: "komainu_api.collateral", expectedEveryMins: 10 },
 ...
 * TODO(CONFIRM-KOMAINU-OPENAPI): replace with a per-endpoint allowlist once
 * docs/phase1/komainu-openapi-1.6.0.json is committed.
 */
...
export function requestToRecord(r: KomainuRequest, label: string): SourceRecordInput {
  return {
    externalId: r.id,
...
export function transactionToRecord(t: KomainuTransaction, label: string): SourceRecordInput {
  return {
    externalId: t.id,
    credentialLabel: label,
    status: t.status,
...
async function pollByStatus<T>(
  ...
  pageLimit?: (cred: KomainuCredential, status: string) => Promise<T[]>,
) {
  ...
  if (errors.length) logger.warn("Komainu poll had credential errors", { kind, errors: errors.map((e) => e.label) });
  ...
  if (results.length === 0 && errors.length) throw new Error(`Komainu ${kind} poll failed for every credential`);
...
export function pollTransactions() {
  return pollByStatus<KomainuTransaction>(
    "transaction",
    "/v1/custody/transactions",
    ["PENDING", "BROADCASTED", "FAILED"],
    transactionToRecord,
    KOMAINU_HEARTBEATS.transactions,
    // FAILED is unbounded history: read the first page only.
    // TODO(CONFIRM-KOMAINU-OPENAPI): confirm ordering / a date filter for FAILED.
    async (cred, status) =>
      status === "FAILED"
        ? (await komainuGet<{ data: KomainuTransaction[] }>("/v1/custody/transactions", { status, page: "1", page_size: "100" }, cred)).data ?? []
        : fetchAllPages<KomainuTransaction>("/v1/custody/transactions", { status }, cred),
  );
}
...
export async function pollCollateral() {
  ...
  if (parts.some((p) => p.ok)) {
    await recordHeartbeat(KOMAINU_HEARTBEATS.collateral.source, {
      count: all.length,
      newestRecordAt: newestTimestamp(all),
      expectedEveryMins: KOMAINU_HEARTBEATS.collateral.expectedEveryMins,
    });
  } else if (parts.some((p) => p.errors.length)) {
    throw new Error("Komainu collateral poll failed for every credential");
  }
...
export async function pollAuditLogs(now = new Date()) {
  ...
  const { results, errors } = await forEachCredential<SourceRecordInput>(async (cred) => {
    ...
    for (const category of ["ADMINISTRATION", "TRANSACTIONS"]) {
      // TODO(CONFIRM-KOMAINU-OPENAPI): cursor pagination parameter; first page only until confirmed.
      const res = await komainuGet<{ data?: Array<Record<string, unknown>> }>(
        "/v1/audit-logs",
  ...
  if (results.length > 0) {
    await recordHeartbeat(KOMAINU_HEARTBEATS.auditLogs.source, { count: records.length, newestRecordAt: newestTimestamp(records), expectedEveryMins: KOMAINU_HEARTBEATS.auditLogs.expectedEveryMins });
  } else if (errors.length) {
    throw new Error("Komainu audit-log poll failed for every credential");
  }
...
async function pollDaily(kind: string, path: string, hb: { source: string; expectedEveryMins: number }, idOf: (r: Record<string, unknown>) => string | null) {
  ...
  else if (errors.length) throw new Error(`Komainu ${kind} poll failed for every credential`);
  return { count: records.length };
}

/** EOD balances (MTD and staking support), daily at 07:00 UTC. */
export function pollEodBalances() {
  // TODO(CONFIRM-KOMAINU-OPENAPI): confirm the record identity fields.
  return pollDaily("eod_balance", "/v1/custody/wallets/eodbalances", KOMAINU_HEARTBEATS.eodBalances, (r) =>
