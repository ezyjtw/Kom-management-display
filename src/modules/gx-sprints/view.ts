/** /gx-sprints view model (spec §16.7): dates, items by type, mapping to tasks, UAT status and the gate. */
...
export async function sprintsView() {
  const sprints = await prisma.gxSprint.findMany({ orderBy: { sprint: "desc" }, take: 20 });
  const changes = await prisma.gxChange.findMany({ where: { sprintId: { in: sprints.map((s) => s.id) } }, orderBy: { createdAt: "asc" } });
