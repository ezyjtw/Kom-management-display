/**
 * FAB ICS repo MVP0 (spec §12 TASK-FAB), behind flag module.fab. The process
 * is draft, not operational; the outbound maker role is unresolved.
 *
 * - Instruction register: TRD_NTF and STL_INS rows. Entered from the fab_ics
 *   mailbox until the parser is written from confirmed templates
 *   (TODO(CONFIRM-FAB-TEMPLATES)); no format is guessed.
 ...
 * - One ticket per instruction reference (TODO(CONFIRM-FAB-PROJECT): setting
 *   fab.ticketProject; empty leaves instructions unticketed and reported).
 * Recording an ACK/NACK or a settlement here never sends anything to FAB.
 */
...
export async function createInstruction(input: z.infer<typeof instructionSchema>, actorId: string | null) {
  const row = await prisma.fabInstruction.create({ data: { ...input, receivedAt: new Date(input.receivedAt), createdById: actorId } });
  const projectKey = await getSetting("fab.ticketProject");
  const item = await ensureTicketedWorkItem({
    kind: "fab_instruction",
    title: `FAB ${input.messageType} ${input.reference}: ${input.direction} ${input.amount} ${input.asset} (value ${input.valueDate})`,
    team: "Team 1",
    taskCode: "TASK-FAB",
    sourceSystem: "fab",
    ...
    ticket: projectKey
      ? {
          projectKey,
          summary: `FAB ${input.messageType} ${input.reference}`,
          description: `FAB instruction ${input.reference}\nType: ${input.messageType} ${input.instructionType}\nDirection: ${input.direction}\nAsset: ${input.asset}\nAmount: ${input.amount}\nValue date: ${input.valueDate}\nReceived: ${input.receivedAt}`,
          labels: ["fab", `fab-${input.messageType.toLowerCase()}`],
        }
      : null,
  });
  return prisma.fabInstruction.update({ where: { id: row.id }, data: { workItemId: item.id } });
}

/** Register view with tx hashes masked (H8). */
export async function fabRegister() {
  const [instructions, settlements, balances] = await Promise.all([
    prisma.fabInstruction.findMany({ orderBy: { receivedAt: "desc" }, take: 200 }),
    prisma.fabSettlementLog.findMany({ orderBy: { occurredAt: "desc" }, take: 500 }),
    prisma.fabFeeBalance.findMany({ orderBy: { recordedAt: "desc" }, take: 100 }),
  ]);
  return {
    instructions,
