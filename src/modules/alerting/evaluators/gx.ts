/**
 * GX sprint UAT alerts evaluated on a clock (spec §16.6). The event rules
...
export async function evaluateUatBeforeProd(ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  ...
  const sprints = await prisma.gxSprint.findMany({ where: { prodPlannedAt: { not: null } } });
  ...
  for (const s of sprints) {
    ...
    out.push({
      dedupeKey: s.sprint,
      severity: "high",
      title: `GX Sprint ${s.sprint}: UAT not complete before PROD`,
...
/** ALR-HB-GXNOTES: a sprint has a KMNC UAT ticket but no release-notes page after a day. */
export async function evaluateReleaseNotesMissing(ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  ...
  const sprints = await prisma.gxSprint.findMany({ where: { pageVersion: 0, createdAt: { lt: new Date(ctx.now.getTime() - graceHours * 3_600_000) } } });
  return sprints
    .filter((s) => Array.isArray(s.kmncKeys) && s.kmncKeys.length > 0)
    .map((s) => ({
      dedupeKey: s.sprint,
      severity: "medium" as const,
      title: `GX Sprint ${s.sprint}: release notes not found`,
      detail: `KMNC has change tickets for Sprint ${s.sprint} (${(s.kmncKeys as string[]).join(", ")}) but no "[GX-Orchestrate] Sprint ${s.sprint} Release Notes" page was found. Check the page title and the release-notes parent (CONFIRM-GX-RELEASE-PARENT).`,
