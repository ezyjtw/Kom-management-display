/**
 * FAB rules (spec §11.2 ALR-FAB-01..08) over the FAB instruction register and
 * settlement log (spec §12 TASK-FAB). Nothing is evaluated while module.fab
...
type Instruction = Awaited<ReturnType<typeof prisma.fabInstruction.findMany>>[number];
function linked(i: Instruction): Pick<AlertCandidate, "workItemId" | "workItemSeed"> {
  return i.workItemId ? { workItemId: i.workItemId } : { workItemSeed: { kind: "fab_instruction", team: "Team 1", taskCode: "TASK-FAB" } };
}

/** ALR-FAB-01: instruction received, not yet ACKed or NACKed. */
export async function evaluateInstructionReceived(_ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  if (!(await on())) return [];
  const rows = await prisma.fabInstruction.findMany({ where: { ackStatus: "none" } });
  return rows.map((i) => ({
    dedupeKey: i.sourceMessageId ?? i.reference,
    severity: "medium" as const,
    title: `FAB instruction received: ${i.messageType} ${i.reference}`,
...
/** ALR-FAB-02: no ACK/NACK within the confirmed number of minutes (CONFIRM-FAB-ACK-MINS). */
export async function evaluateNotAcknowledged(ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  ...
  const rows = await prisma.fabInstruction.findMany({ where: { ackStatus: "none", receivedAt: { lt: new Date(ctx.now.getTime() - mins * 60_000) } } });
  return rows.map((i) => ({
    dedupeKey: i.reference,
    severity: "high" as const,
    title: `FAB instruction not acknowledged: ${i.reference}`,
...
/** ALR-FAB-03: instruction received after the cut-off (London). Informational; never auto-resolves. */
export async function evaluateAfterCutoff(ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  ...
  const rows = (await prisma.fabInstruction.findMany({ where: { receivedAt: { gte: since } } }))
    .filter((i) => i.receivedAt >= londonInstant(londonParts(i.receivedAt).date, cutoff));
  return rows.map((i) => ({
    dedupeKey: i.reference,
    severity: "medium" as const,
    title: `FAB instruction after cut-off: ${i.reference}`,
...
/** ALR-FAB-04: NACK sent; clears when a corrected instruction is recorded. */
export async function evaluateNackSent(_ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  if (!(await on())) return [];
  const rows = await prisma.fabInstruction.findMany({ where: { ackStatus: "NACK", correctedByRef: null } });
  return rows.map((i) => ({
    dedupeKey: i.reference,
    severity: "medium" as const,
    title: `FAB NACK sent: ${i.reference}`,
...
async function instructionsByRef(refs: string[]) {
  return new Map((await prisma.fabInstruction.findMany({ where: { reference: { in: refs } } })).map((i) => [i.reference, i]));
}

/** ALR-FAB-05: settlement FAILED in the log. Never auto-resolves (needs write-up); opens an IAI draft. */
export async function evaluateSettlementFailed(_ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  if (!(await on())) return [];
  const failed = await prisma.fabSettlementLog.findMany({ where: { status: "FAILED" } });
  ...
  return [...new Set(failed.map((f) => f.reference))].map((ref) => {
    ...
    return {
      ...
      title: `FAB settlement failed: ${ref}`,
      detail: `The settlement log records FAILED for instruction ${ref}.`,
      ...(i ? linked(i) : { workItemSeed: { kind: "fab_instruction" as const, team: "Team 1", taskCode: "TASK-FAB" } }),
    };
  });
}

/** ALR-FAB-06: OPEN or MARGIN RECEIVE instruction with value date today and no inbound recorded by the cut-off. */
export async function evaluateDepositNotReceived(ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  ...
  const due = await prisma.fabInstruction.findMany({ where: { valueDate: today, instructionType: { in: ["OPEN", "MARGIN_RECEIVE", "MARGIN RECEIVE"] }, direction: "RECEIVE" } });
  if (!due.length) return [];
  const inbound = new Set((await prisma.fabSettlementLog.findMany({ where: { reference: { in: due.map((d) => d.reference) }, status: { in: ["RECEIVED", "COMPLETED"] } } })).map((l) => l.reference));
  return due.filter((i) => !inbound.has(i.reference)).map((i) => ({
    dedupeKey: i.reference,
    severity: "high" as const,
    title: `FAB deposit not received by value date: ${i.reference}`,
    detail: `${i.amount} ${i.asset} due today (${today}); no inbound recorded by the cut-off.`,
    ...linked(i),
  }));
}

/** ALR-FAB-07: inbound with a KYT fail or escalation; clears when the Compliance outcome is recorded. */
export async function evaluateInboundKytLock(_ctx: EvaluatorContext): Promise<AlertCandidate[]> {
  if (!(await on())) return [];
  const rows = await prisma.fabSettlementLog.findMany({ where: { kytStatus: { in: ["fail", "escalated"] } } });
  ...
  return rows.map((r) => {
    ...
    return {
      dedupeKey: r.id,
      severity: "critical" as const,
      title: `FAB inbound KYT lock: ${r.reference}`,
      detail: `KYT ${r.kytStatus} on an inbound for ${r.reference}. FAB inbound collateral is not risk-scored; Compliance outcome required.`,
      ...(i ? linked(i) : { workItemSeed: { kind: "fab_instruction" as const, team: "Team 1", taskCode: "TASK-FAB" } }),
    };
  });
}
