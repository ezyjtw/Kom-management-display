/**
 * Spec §16.4: compare a newly supplied the custody provider OpenAPI spec with the committed
 ...
 *
 *   npx tsx scripts/check-custody-api-spec.ts <new-spec.json> [--committed docs/phase1/custody-openapi-1.6.0.json]
 *
...
if (!newPath || newPath.startsWith("--")) {
  console.error("Usage: npx tsx scripts/check-custody-api-spec.ts <new-spec.json> [--committed <path>]");
  process.exit(2);
}
// TODO(CONFIRM-CUSTODY-OPENAPI): the committed spec file.
const committedPath = arg("committed") ?? "docs/phase1/custody-openapi-1.6.0.json";
...
if (existsSync(committedPath)) {
  ...
  console.log(`No committed spec at ${committedPath} (CONFIRM-CUSTODY-OPENAPI); checking the contract only.\n`);
}
