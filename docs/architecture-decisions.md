...
**Context**: Production runs on Azure Database for PostgreSQL or, since 2026-09-25, Railway PostgreSQL (H10 removed; see docs/phase1/go-live.md). Local development uses PostgreSQL (via Docker or native install).
...
**Decision**: Append-only AuditLog table with JSON details. Enforced in the database since the baseline migration (triggers reject UPDATE, DELETE and TRUNCATE; an insert trigger normalises the actor). Control-relevant actions are audited fail-closed via `auditedAction()` (requested → completed/failed, linked by `correlationId`).
