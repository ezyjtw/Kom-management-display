...
| 2 | Semgrep `detect-non-literal-regexp` | Platform impact rules (`platform-sprints/mapping.ts`), ALR-CFG-01 event patterns (`alerting/evaluators/operations.ts`), import filename patterns | **Real (low to medium).** Admin-authored regexes ran against external text (Confluence release notes, the custody provider audit events). A catastrophic pattern could stall the worker (ReDoS). | `src/lib/safe-regex.ts`: patterns with nested quantifiers or backreferences are refused when saved and ignored when run, and the text tested is capped at 20k characters. Test: `security-scan-fixes`. |
