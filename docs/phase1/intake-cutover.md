...
2. **Configure** (Admin → Client Intake):
   ...
   - the custody provider's Slack workspace id (`kommand` route);
...
3. **Map clients** (Admin → Clients & Channels): JSM organisation id, Slack
   ...
   who post from the custody provider's workspace.
4. **Register Slack channels** with purpose `client` and the linked client.
5. For email: set the custody provider's internal email domains, then enable email intake.
   For Teams: set the custody provider's tenant id, then enable Teams intake.

## Cut-over

1. Enable the chosen route at a quiet time; watch the first few requests.
2. **Switch off the `:inbox_tray:` Slack-to-VND skill** (the emoji-triggered
