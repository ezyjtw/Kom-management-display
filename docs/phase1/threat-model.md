...
| Demo tier (`KOM_ENVIRONMENT=demo`, or Railway with it unset) takes secrets from environment variables and may allow local login and seeding | A hosted demo on synthetic data (Railway) cannot mount Key Vault files, and the owner wants a demo before the APIs are connected. | Only when named explicitly, or on Railway with `KOM_ENVIRONMENT` unset (Phase 12o): `KOM_ENVIRONMENT=production` always wins. Elsewhere a production build defaults to the production tier. On the Railway demo, local login is on unless `ALLOW_LOCAL_LOGIN=false`, and a pre-baseline demo database is rebuilt unless `KOM_DEMO_RESET_LEGACY=false`. Every page shows a DEMO banner. Never connected to live systems (H9). A production-tier start refuses a database carrying the demo marker, and `go-live:check` fails on the demo tier, environment-variable secrets, local login, seeding or seeded identities. | Accepted by the owner, 2026-09-24 (`go-live.md`) |
| Production may run on Railway (hard constraint H10 removed) | Owner decision, 2026-09-25: Railway is acceptable as the production host for now. Railway cannot mount Key Vault files, has no managed identities for the database and no private endpoints. | Secrets are Railway sealed variables, read only from the environment of that service; `KOM_ENVIRONMENT=production` must be set explicitly (the Railway default is demo); Entra SSO only, no local login, never seeded, refuses a demo-marked database; the app-level egress allowlist still applies; `go-live:check` runs inside the service. The database password in `DATABASE_URL` and the missing network isolation are accepted residual risks. | Accepted by the owner, 2026-09-25 (`go-live.md`); review before any client data is loaded |
